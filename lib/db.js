...
module.exports = function (opt, cb) {
    ...
    async.series({
        ...
        history: require('./db/history').bind(this, db, opt)
    }, function (err, res) {
    ...
    self.stats = function (cb) {
        async.parallel({
            words: db.get.bind(db, 'select count(*) as counter from dict;', []),
            associations: db.get.bind(db, 'select count(*) as counter from assoc;', []),
            ngrams: db.get.bind(db, 'select count(*) as counter from markov;', [])
        }, cb);
    };
